...
import { LogOut, Plus, Trash2, Save, Upload, User, Briefcase, GraduationCap, Code2, FolderKanban, Mail, ExternalLink, Github } from 'lucide-react';
import { authFetch } from '../lib/api';
import { useAuth } from '../contexts/AuthContext';
import { playClickSound } from '../lib/sound';

type Tab = 'profile' | 'experience' | 'education' | 'skills' | 'projects' | 'messages';
...
export default function Admin() {
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    try {
      const [p, e, ed, s, pr, m] = await Promise.all([
